"""
Threat detection and analysis
Identifies suspicious patterns and behaviors
"""

import re
from typing import Dict, List


class ThreatDetector:
    """Analyzes data for security threats"""
    
    # Suspicious command patterns
    SUSPICIOUS_COMMANDS = [
        r'cat\s+/etc/(passwd|shadow)',
        r'sudo\s+',
        r'rm\s+-rf',
        r'chmod\s+',
        r'chown\s+',
        r'wget\s+',
        r'curl\s+',
        r'nc\s+',
        r'bash\s+',
        r'sh\s+',
        r'python\s+',
        r'perl\s+',
        r'dd\s+',
        r'mkfs\s+',
        r'mount\s+',
        r'iptables\s+',
        r'ssh-keygen\s+',
    ]
    
    # SSH suspicious patterns
    SSH_SUSPICIOUS_PATTERNS = [
        r'root@',
        r'admin@',
        r'test@',
        r'SSH.*version.*OpenSSH',
    ]
    
    # Brute force detection
    BRUTE_FORCE_THRESHOLD = 5  # Failed attempts before flagging
    
    def __init__(self):
        """Initialize threat detector"""
        self.compiled_commands = [re.compile(pattern) for pattern in self.SUSPICIOUS_COMMANDS]
        self.compiled_ssh = [re.compile(pattern) for pattern in self.SSH_SUSPICIOUS_PATTERNS]
        self.failed_attempts = {}  # Track failed attempts per IP
    
    def analyze_command(self, command: str) -> int:
        """
        Analyze command for threats
        Returns threat level 0-5 (0 = safe, 5 = critical)
        """
        threat_level = 0
        command_lower = command.lower()
        
        # Check against suspicious patterns
        for pattern in self.compiled_commands:
            if pattern.search(command_lower):
                threat_level = max(threat_level, 4)  # HIGH
                break
        
        # Check for common exploit attempts
        if any(indicator in command_lower for indicator in ['exploit', 'shellcode', 'payload']):
            threat_level = max(threat_level, 5)  # CRITICAL
        
        # Check for file operations on sensitive paths
        if any(path in command_lower for path in ['/etc/shadow', '/etc/passwd', '/root', '/.ssh']):
            threat_level = max(threat_level, 4)  # HIGH
        
        # Check for privilege escalation attempts
        if any(priv in command_lower for priv in ['sudo -i', 'sudo su', 'su -']):
            threat_level = max(threat_level, 4)  # HIGH
        
        # Check for common IoT botnet commands
        if any(bot in command_lower for bot in ['mirai', 'dofloo', 'tsunami']):
            threat_level = max(threat_level, 5)  # CRITICAL
        
        return threat_level
    
    def analyze_ssh(self, ssh_data: str) -> int:
        """
        Analyze SSH handshake/data for threats
        Returns threat level 0-5
        """
        threat_level = 0
        ssh_data_lower = ssh_data.lower()
        
        # Check for suspicious SSH patterns
        for pattern in self.compiled_ssh:
            if pattern.search(ssh_data_lower):
                threat_level = max(threat_level, 2)  # LOW
                break
        
        # Check for version scanning
        if 'openssh' in ssh_data_lower or 'libssh' in ssh_data_lower:
            threat_level = max(threat_level, 1)  # INFO
        
        # Check for known exploit signatures
        if any(exploit in ssh_data_lower for exploit in ['cve-2018', 'cve-2019', 'rce']):
            threat_level = max(threat_level, 4)  # HIGH
        
        return threat_level
    
    def analyze_payload(self, data: bytes) -> int:
        """
        Analyze binary payload for threats
        Returns threat level 0-5
        """
        threat_level = 0
        
        # Check for executable headers
        if data.startswith(b'\x7fELF'):  # ELF binary
            threat_level = max(threat_level, 4)  # HIGH
        
        if data.startswith(b'MZ'):  # Windows PE
            threat_level = max(threat_level, 4)  # HIGH
        
        # Check for shellcode patterns
        if self._contains_shellcode_pattern(data):
            threat_level = max(threat_level, 4)  # HIGH
        
        return threat_level
    
    def _contains_shellcode_pattern(self, data: bytes) -> bool:
        """Detect common shellcode patterns"""
        shellcode_patterns = [
            b'\xff\xe4',  # jmp esp
            b'\x90\x90\x90',  # NOP sled
            b'/bin/sh',
            b'/bin/bash',
        ]
        
        for pattern in shellcode_patterns:
            if pattern in data:
                return True
        
        return False
    
    def check_brute_force(self, client_ip: str, failed: bool = False) -> bool:
        """
        Track and detect brute force attempts
        Returns True if brute force is detected
        """
        if client_ip not in self.failed_attempts:
            self.failed_attempts[client_ip] = 0
        
        if failed:
            self.failed_attempts[client_ip] += 1
        else:
            self.failed_attempts[client_ip] = 0
        
        if self.failed_attempts[client_ip] >= self.BRUTE_FORCE_THRESHOLD:
            return True
        
        return False
    
    def analyze_http_request(self, method: str, path: str, headers: Dict) -> int:
        """
        Analyze HTTP request for threats (for future HTTP honeypot)
        Returns threat level 0-5
        """
        threat_level = 0
        
        # SQL injection patterns
        sql_indicators = ['union', 'select', 'insert', 'update', 'delete', 'drop', 'exec']
        if any(indicator in path.lower() for indicator in sql_indicators):
            threat_level = max(threat_level, 3)  # MEDIUM
        
        # Path traversal
        if '../' in path or '..\\' in path:
            threat_level = max(threat_level, 3)  # MEDIUM
        
        # Command injection
        if any(char in path for char in ['|', ';', '&', '$', '`']):
            threat_level = max(threat_level, 3)  # MEDIUM
        
        # Dangerous HTTP methods with high port
        if method in ['DELETE', 'PUT'] and len(path) > 1000:
            threat_level = max(threat_level, 2)  # LOW
        
        return threat_level
